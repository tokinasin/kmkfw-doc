...
Known working and recommended devices can be found in the [list of officially supported microcontrollers](/docs/basics/Officially_Supported_Microcontrollers)
...
![feather and keeboar example pins](/docs/picture/pins56.jpg)
...
You will need the `kb.py` and `main.py`. If you need more detailed instructions on how to customize the configuration settings and key mappings, please refer to the [config and keymap](/docs/basics/config_and_keymap) documentation.
...
- To start customizing your `code.py`/`main.py` file, please refer to the [config and keymap](/docs/basics/config_and_keymap) and [keys](/docs/advanced/keys) files respectively, which provide detailed instructions on how to modify the configuration settings and key mappings.
- There's a [reference](/docs/basics/keycodes) of the available keycodes
- [International](/docs/extensions/international) extension adds keys for non US layouts and [Media Keys](/docs/extensions/media_keys) adds keys for ... media

And to go even further:
- [Macros](/docs/modules/macros) are used for sending multiple keystrokes in a single action
- [Layers](/docs/modules/layers) can transform the whole way your keyboard is behaving with a single touch
- [HoldTap](/docs/modules/holdtap) allow you to customize the way a key behaves whether it is tapped or hold, and [TapDance](/docs/modules/tapdance) depending on the number of times it is pressed

Want to have fun features such as RGB, split keyboards and more? Check out what builtin [modules](/docs/modules) and [extensions](/docs/extensions) can do!
...
In case you need it, debugging help can be found on the [debugging](/docs/basics/debugging) page.
